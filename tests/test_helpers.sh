...
# way a shared guard stays one file rather than five diverging copies. The
# bodies use this repository's own loggers.
CHECKS_FAILED=0
...
wait_for_log() {
    ...
    while [[ ${elapsed} -lt ${timeout} ]]; do
        ...
        # Also check the palworld server log file inside container
        if docker_exec "${container}" grep -qi "${pattern}" /var/log/palworld/palworld-server.log 2>/dev/null; then
            return 0
        fi
        # Check supervisor stdout log
        if docker_exec "${container}" grep -qi "${pattern}" /var/log/palworld/supervisor-palworld.log 2>/dev/null; then
            return 0
        fi
        sleep 5
        elapsed=$((elapsed + 5))
